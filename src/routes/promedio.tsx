...
/* ================================================================
   CONSTANTES
================================================================ */
const NOTA_COLOR: Record<number, string> = { 1: "#f87171", 2: "#a78bfa", 3: "#3b82f6", 4: "#22d3ee", 5: "#34d399" };
const NOTA_LABEL: Record<number, string> = { 1: "Insuf.", 2: "Regular", 3: "Bueno", 4: "Distint.", 5: "Sobres." };
...
function PromedioPage() {
  ...
  const semestres = useMemo(() => {
    const map = new Map<string, { id: string; materia: string; semestre: string; plan: string }[]>();
    materias.forEach(m => {
    ...
    return [...map.entries()]
      ...
      .map(([sem, items]) => ({ sem, items: items.sort((a, b) => a.materia.localeCompare(b.materia, "es")) }));
  }, [materias]);
  ...
  useEffect(() => {
    if (!hydrated) return;
    try { writeLocalState(STORAGE_KEY, JSON.stringify(notas)); } catch { /* ignore */ }
  }, [notas, hydrated]);
  ...
  const stats = useMemo(() => {
    ...
    const distribucion = [1, 2, 3, 4, 5].map(nota => ({
      nota, cantidad: valoresPonderados.filter(v => v === nota).length,
    }));
  ...
  const promedioColor = stats
    ? stats.promedio >= 4 ? "#34d399" : stats.promedio >= 3 ? "#3b82f6" : stats.promedio >= 2 ? "#a78bfa" : "#f87171"
    : "#8b97c2";
          ...
          <div className="mx-auto max-w-6xl px-6">
            <Reveal className="max-w-3xl">
              ...
              <p className="mt-4 max-w-xl leading-relaxed text-muted-foreground">
                Marcá las materias que ya aprobaste y elegí la nota — el listado sale directo
                del plan de materias, sin tener que escribir nada.
              </p>
            </Reveal>
          </div>
        </section>
                    ...
                    <div className="card-hover rounded-2xl border border-border bg-card p-6">
                      {stats ? (
                        <>
                          <div className="mb-5 grid grid-cols-3 gap-4">
                            <div className="text-center">
                              <p className="text-3xl font-black text-foreground">{stats.materiasCount}</p>
                            ...
                            <div className="text-center">
                              <p className="text-3xl font-black tabular-nums" style={{ color: promedioColor }}>
                          ...
                          <div className="mb-3 h-2.5 w-full overflow-hidden rounded-full bg-foreground/10">
                            <div className="h-full rounded-full transition-all duration-700"
                              style={{ width: `${(stats.promedio / 5) * 100}%`, background: promedioColor }} />
                          </div>
                              ...
                              <div key={nota} className="text-center">
                                <div className="relative mb-1 flex h-14 items-end justify-center">
                                  <div
                                    ...
                                    style={{
                                      height: stats.divisor ? `${Math.max(4, (cantidad / stats.divisor) * 56)}px` : "4px",
                                      background: `${NOTA_COLOR[nota]}80`,
                                      border: `1px solid ${NOTA_COLOR[nota]}60`,
                                    }}
                                  />
                                </div>
                                <p className="text-xs font-bold" style={{ color: NOTA_COLOR[nota] }}>{nota}</p>
                          ...
                          {stats.materiasNota1 > 0 && (
                            <div className="mt-4 flex items-start gap-2 rounded-xl border border-amber-500/25 bg-amber-500/8 px-3.5 py-3 text-xs text-muted-foreground">
                              ...
                              <span>
                                <strong className="text-foreground">Regla del reglamento aplicada:</strong>{" "}
                                {stats.materiasNota1 === 1 ? "1 materia con nota 1 se cuenta" : `${stats.materiasNota1} materias con nota 1 se cuentan`} dos veces
                                en el promedio (se recursa y vuelve a computarse al aprobarla). Por eso el promedio usa {stats.divisor} valores en vez de {stats.materiasCount}.
                              </span>
                            </div>
                          )}
                        </>
                        ...
                        <div className="py-6 text-center text-muted-foreground">
                          ...
                          <p className="text-sm font-medium text-foreground">Todavía no marcaste materias</p>
                          <p className="mt-1 text-xs">Elegí materias de la lista para ver tu promedio acá.</p>
                        </div>
                      )}
                    </div>
                    {stats && (
              ...
              <div className="order-1 space-y-5 lg:order-2 lg:col-span-3">
                ...
                >
                  {semestresFiltrados.map(({ sem, items }) => {
                    ...
                    return (
                      <Reveal key={sem}>
                        <AccordionItem value={sem} className="rounded-2xl border border-border bg-card px-4 border-b-0">
                          <AccordionTrigger className="py-3.5 hover:no-underline">
                            <span className="flex items-center gap-2.5">
                              <span className="text-sm font-semibold text-foreground">
                                {sem === "—" ? "Sin semestre asignado" : `${sem}° Semestre`}
                              </span>
                              <span className="rounded-full bg-muted/60 px-2 py-0.5 text-[11px] text-muted-foreground">{items.length} materias</span>
                              {aprobadasEnSemestre > 0 && (
                                <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[11px] font-medium text-primary">{aprobadasEnSemestre} aprobadas</span>
                              )}
                            </span>
                          </AccordionTrigger>
                          <AccordionContent>
                              ...
                              {items.map(({ id, materia, plan }) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    >
                                      <span className={`grid h-5 w-5 flex-shrink-0 place-items-center rounded-md border transition-colors ${aprobada ? "border-primary bg-primary" : "border-border"}`}>
                                        {aprobada && <Check className="h-3.5 w-3.5 text-primary-foreground" />}
                                      </span>
                                      <span className="flex-1 text-sm font-medium text-foreground">{materia}<span className="ml-2 text-[10px] font-normal text-muted-foreground">Plan {plan}</span></span>
                                      {aprobada && (
                                        <span
                                          className="rounded-full px-2 py-0.5 text-[11px] font-bold"
                                          style={{ background: `${NOTA_COLOR[nota]}20`, color: NOTA_COLOR[nota] }}
                                        >
                                          {nota} — {NOTA_LABEL[nota]}{nota === 1 ? " · cuenta doble" : ""}
                                        </span>
                                      )}
                                    </button>
                                    {aprobada && (
                                      <div className="flex gap-1.5 px-4 pb-3">
                                        {[1, 2, 3, 4, 5].map(v => (
                                          <button
                                            ...
                                            style={
                                              nota === v
                                                ? { background: `${NOTA_COLOR[v]}25`, color: NOTA_COLOR[v], border: `2px solid ${NOTA_COLOR[v]}` }
                                                : { background: "var(--muted)", color: "var(--muted-foreground)", border: "2px solid transparent" }
                                            }
                                          >
                                            {v}
                                          </button>
                  ...
                  {semestresFiltrados.length === 0 && (
                    <p className="py-10 text-center text-sm text-muted-foreground">No se encontraron materias.</p>
                  )}
                </Accordion>
              </div>
            </div>
