...
import {
  ChevronRight, CalendarDays, Users, GraduationCap, PartyPopper,
  ClipboardList, Globe2, FileCheck2, ArrowRight,
} from "lucide-react";
import { Reveal } from "@/components/Reveal";
import { SiteNavbar } from "@/components/SiteNavbar";
import { SiteFooter } from "@/components/SiteFooter";
import {
  EVENTOS_CON_FECHA, TRAMITES, HOMOLOGACIONES, PERIODOS_EVALUACION, ENTREGA_DOCUMENTOS,
  estadoPeriodo, fechaPeriodoLabel, type CategoriaCalendario, type PeriodoAcademico,
} from "@/data/calendario-academico";

export const Route = createFileRoute("/calendario-academico")({ component: CalendarioAcademicoPage });

const CATEGORIA_META: Record<CategoriaCalendario, { label: string; color: string; Icon: typeof Users }> = {
...
function PeriodoCard({ periodo, compact = false }: { periodo: PeriodoAcademico; compact?: boolean }) {
  ...
  return (
    <div className={`rounded-xl border border-border bg-card ${compact ? "p-3" : "p-4"}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="flex-shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold" style={{ background: `${meta.color}1f`, color: meta.color }}>
          {meta.label}
        </span>
      </div>
    </div>
...
function CalendarioAcademicoPage() {
  ...
  const eventos = useMemo(
    () => (filtro === "todos" ? EVENTOS_CON_FECHA : EVENTOS_CON_FECHA.filter(e => e.categoria === filtro)),
    [filtro],
  );
          ...
          <div className="mx-auto max-w-6xl px-6">
            <Reveal className="max-w-3xl">
              ...
              <p className="mt-4 max-w-xl leading-relaxed text-muted-foreground">
                Fechas clave del período, trámites disponibles, exámenes, homologaciones y
                feriados — todo en un solo lugar.
              </p>
            </Reveal>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-5xl px-6 space-y-16">
            ...
            <div>
                  ...
                  {FILTROS.map(f => (
                    <button
                      ...
                      className={`rounded-full px-3.5 py-1.5 text-xs font-medium transition-colors ${
                        filtro === f.key ? "bg-primary text-primary-foreground" : "border border-border bg-card text-muted-foreground hover:text-foreground"
                      }`}
                    >
                      {f.label}
                    </button>
              ...
              <div className="relative space-y-3 border-l-2 border-border pl-6">
                    ...
                    <Reveal key={e.id}>
                      <div className="card-hover relative rounded-2xl border border-border bg-card p-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide" style={{ background: `${meta.color}20`, color: meta.color }}>
                            {meta.label}
                          </span>
                ...
                {eventos.length === 0 && (
                  <p className="py-6 text-sm text-muted-foreground">No hay eventos para este filtro.</p>
                )}
              </div>
            </div>
            ...
            <Reveal>
              <div>
                ...
                <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
                  Todos los procesos inician y finalizan a las 12:00 h del mediodía. Los pedidos posteriores se procesan para el siguiente periodo académico.
                </p>
              </div>
            </Reveal>
            ...
            <Reveal>
              <div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {PERIODOS_EVALUACION.map(periodo => <PeriodoCard key={periodo.id} periodo={periodo} />)}
                </div>
              </div>
            </Reveal>
              ...
              <Reveal>
                <div className="card-hover h-full rounded-2xl border border-border bg-card p-5">
                  ...
                  <p className="mt-1.5 text-sm text-muted-foreground">
                    Los periodos generales están indicados arriba. El día, la hora y el aula dependen de cada materia.
                  </p>
                  <Link to="/aulas" className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-primary hover:underline">
              ...
              <Reveal>
                <div className="card-hover h-full rounded-2xl border border-border bg-card p-5">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {HOMOLOGACIONES.map(h => <PeriodoCard key={h.id} periodo={h} compact />)}
                  </div>
                </div>
              </Reveal>
              <Reveal>
                <div className="card-hover h-full rounded-2xl border border-border bg-card p-5">
                  ...
                  <div className="mt-2"><PeriodoCard periodo={ENTREGA_DOCUMENTOS} compact /></div>
                </div>
              </Reveal>
            </div>
          </div>
