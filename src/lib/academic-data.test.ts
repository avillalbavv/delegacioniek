...
test("Dibujo Técnico y Dibujo Técnico Mecánico permanecen separados", () => {
  ...
  const mecanico = rows.filter(row => normalizeAcademicName(row.materia) === "dibujo tecnico mecanico");
  assert.ok(tecnico.length > 0);
  assert.ok(mecanico.length > 0);
  assert.equal(new Set([...tecnico, ...mecanico].map(row => row.id)).size, tecnico.length + mecanico.length);
...
test("los semestres académicos corregidos están en la fuente central", () => {
  const semesters = (name: string) => new Set(
    rows.filter(row => normalizeAcademicName(row.materia) === normalizeAcademicName(name)).map(row => String(row.semGrupo)),
  );
...
test("la identidad académica compuesta no contiene duplicados", () => {
  const identities = rows.map(row => [
    normalizeAcademicName(row.materia), row.plan, row.seccion, row.turno,
    String(row.semGrupo), row.enfasis,
  ].join("::"));
...
test("la delegación mantiene cargos y orden por apellido", () => {
  ...
  const ordered = ["Ávalos", "Cáceres", "Echeverría", "Ferreira", "Florentín", "Lemir", "Martín", "Martínez", "Martínez", "Matiauda", "Mendoza", "Saldívar", "Villalba"];
  let cursor = 0;
