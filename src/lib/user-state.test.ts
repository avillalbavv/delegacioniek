...
test("la sincronización elige la versión más reciente por herramienta", () => {
  const local = state(
    ...
    { "iek-asistencia:v2": "2026-07-12T02:00:00.000Z", "iek-promedio-db-v2": "2026-07-10T02:00:00.000Z" },
  );
  const remote = state(
    { "iek-asistencia:v2": "asistencia-vieja", "iek-promedio-db-v2": "promedio-remoto" },
    { "iek-asistencia:v2": "2026-07-11T02:00:00.000Z", "iek-promedio-db-v2": "2026-07-12T03:00:00.000Z" },
  );
...
test("la sincronización combina herramientas de dispositivos diferentes", () => {
  const merged = mergeStates(
    ...
    state({ "poliplanner:seleccion:v2": "P" }, { "poliplanner:seleccion:v2": "2026-07-12T01:00:00.000Z" }),
  );
